...
p_dominating_sound <- ggplot(D_long, aes(x = HM_NOISE, y = Value, color = Sound_Type)) +
...
p_soundscate_attr <- ggplot(D_long, aes(x = HM_NOISE, y = Value, color = Attribute)) +
  geom_smooth(method = "gam", se = TRUE, alpha = 0.3, size = 1.2, formula = y ~ s(x, bs = "cs"))
...
vars_tmp <- c("LNOISE", "LSANNOY1_num", "LSANNOY2_num", 
...
D_long_annoyance$Annoyance <- factor(D_long_annoyance$Annoyance, 
  levels = c("In General", "Road Traffic", "Public Transport", 
             "Train", "Plane", "Freetime", "Music of others", "Works")
)
p_noise_annoy <- ggplot(D_long_annoyance, aes(x = HM_NOISE, y = Value, color = Annoyance)) +
  geom_smooth(method = "gam", se = TRUE, alpha = 0.3, size = 1.2, formula = y ~ s(x, bs = "cs"))
```
