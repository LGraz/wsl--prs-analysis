...
ggplot(D.plot, aes(x = HM_NOISE, y = Value, color = Variable)) +
  ...
  guides(color=guide_legend(title="PRS variable"))#+ theme(legend.position = "bottom")


# reproduce the plot above with the long format data
p_RLNOISE_PRS <- ggplot(D.plot, aes(x = RL_NOISE, y = Value, color = Variable)) +
  geom_vline(xintercept = c(45, 55), linetype = "dashed", color = "gray", linewidth = 1.3) +
  geom_smooth(method = "gam", se = TRUE, alpha = 0.25, linewidth = 1.5, formula = y ~ s(x, bs = "cs")) +
  scale_color_discrete(labels = c(
      "BA" = "BA: Being away",
  ...
  ylab("Response [1-7]") #+ labs(title = "d. Perceived Restorativeness Scale (PRS)")


p_RLNOISE_PRS + guides(color=guide_legend(title="PRS variable")) +
  xlab("RL-NOISE Level [dB]") # + theme(legend.position = "bottom")
```
...
ggplot(D, aes(x = HM_NOISE, y = RL_NOISE)) +
  geom_vline(xintercept = c(45, 55), linetype = "dashed", color = "gray", linewidth = 1.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black", linewidth = 1) +
  geom_point(alpha = 0.3) +
